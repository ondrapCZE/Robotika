...
MobDifferentialChassis::MobDifferentialChassis(
	...
	loopPidThread_ = std::move(
			std::thread(&MobDifferentialChassis::updateEncoders, this, 5));
...
VelocityWheels MobDifferentialChassis::computeSpeed(DistanceWheels distance,
		double time) {
...
void MobDifferentialChassis::changeRobotState(DistanceWheels change) {
	double angleChange = (change.right - change.left)
			/ diffChassisParam_.wheelBase;
	...
	robotState_.x += distanceChange
			* cos(robotState_.theta + (angleChange / 2.0f));
	robotState_.y += distanceChange
			* sin(robotState_.theta + (angleChange / 2.0f));
...
int MobDifferentialChassis::PIRegulator(const double actualSpeed,
		const double desireSpeed, PIValue &PIParam) {
	...
	// set in boundaries
	return rob_fce::valueInRange<int>(speed,
			diffChassisParam_.driver->getMaxPower());
}
...
void MobDifferentialChassis::updateEncoders(const int period) {
	...
	while (!end_.load()) {
		...
		valueMotors.left = PIRegulator(actualSpeed.left, copyDesSpeed.left,
				diffChassisParam_.pidLeft);
		valueMotors.right = PIRegulator(actualSpeed.right, copyDesSpeed.right,
				diffChassisParam_.pidRight);
	...
	if (slow) {
		do {
			...
			if (wheelsSpeed_.left > 0) {
				desire.left = rob_fce::valueInRange<double>(
						wheelsSpeed_.left - STOP_SPEED, 0.0, wheelsSpeed_.left);
			} else {
				desire.left = rob_fce::valueInRange<double>(
						wheelsSpeed_.left + STOP_SPEED, wheelsSpeed_.left, 0.0);
			}

			if (wheelsSpeed_.right > 0) {
				desire.right = rob_fce::valueInRange<double>(
						wheelsSpeed_.right - STOP_SPEED, 0.0,
						wheelsSpeed_.right);
			} else {
				desire.right = rob_fce::valueInRange<double>(
						wheelsSpeed_.right + STOP_SPEED, wheelsSpeed_.right,
						0.0);
			}

			setVelocity(desire);
			std::this_thread::sleep_for(std::chrono::milliseconds(10));
		} while (std::abs(wheelsSpeed_.left) > 1e-5
				|| std::abs(wheelsSpeed_.right) > 1e-5);
	} else {
...
void MobDifferentialChassis::setVelocity(const VelocityWheels speed) {
	...
	safeSpeed.left = rob_fce::valueInRange(speed.left,
			diffChassisParam_.maxVelocity);
	safeSpeed.right = rob_fce::valueInRange(speed.right,
			diffChassisParam_.maxVelocity);
...
void MobDifferentialChassis::setVelocity(const double distance,
		const double angle) {
	...
	if (std::abs(angle) < EPSILON) {
		...
		double ratio = (radius - diffChassisParam_.wheelBase / 2.0)
				/ (radius + diffChassisParam_.wheelBase / 2.0);
		if (angle >= 0) {
