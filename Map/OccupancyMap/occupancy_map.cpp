...
OccupancyMap::OccupancyMap(const float resolution, const Size size) :
		resolution_(resolution), sizeX_(size.x / resolution + 1), sizeY_(
				size.y / resolution + 1) {
	...
	for (unsigned int i = 0; i < sizeX_; i++) {
		for (unsigned int j = 0; j < sizeY_; j++) {
			map_[j * sizeX_ + i] = DEFAULT_PROB;
		}
	}
}

OccupancyMap::OccupancyMap(const OccupancyMap& map) :
		sizeX_(map.sizeX_), sizeY_(map.sizeY_), resolution_(map.resolution_) {
	map_ = new float[sizeX_ * sizeY_];

	// copy map values
	for (unsigned int i = 0; i < sizeX_; i++) {
		for (unsigned int j = 0; j < sizeY_; j++) {
			map_[j * sizeX_ + i] = map.map_[j * sizeX_ + i];
		}
	}
}
...
Size OccupancyMap::size() {
	return Size(((float) sizeX_ - 1) * resolution_,
			((float) sizeY_ - 1) * resolution_);
}
...
void OccupancyMap::copy(const OccupancyMap &map){
	if(sizeX_*sizeY_ != map.sizeX_*map.sizeY_){ // reallocate memory for map
		delete[] map_;
		map_ = new float[map.sizeX_*map.sizeY_];
	}
